...
public partial class ProgramsMenu : UserControl
{
    ...
    private void UserControl_Loaded(object sender, RoutedEventArgs e)
    {
        if (!hasLoaded)
        {
            ...
            int i = categorizedProgramsList.Items.IndexOf(
                MenuBar._appGrabber.CategoryList.GetCategory(Settings.Instance.DefaultProgramsCategory));
    ...
    private void btnUninstallApps_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!ShellHelper.StartProcess("appwiz.cpl"))
            CairoMessage.Show(DisplayString.sError_CantOpenAppWiz, DisplayString.sError_OhNo, MessageBoxButton.OK,
        ...
        foreach (Control item in menu.Items)
        {
            ...
            switch (item.Name)
            {
                ...
                case "miProgramsItemRunAs":
                    item.Visibility = KeyboardUtilities.IsKeyDown(System.Windows.Forms.Keys.ShiftKey) && !app.IsStoreApp
                        ? Visibility.Visible
    ...
    private void programsMenu_Rename(object sender, RoutedEventArgs e)
    {
        ...
        MenuBar._appGrabber.RenameAppDialog(app, (bool? result) => { MenuBar.OpenProgramsMenu(); });
    }
    ...
    private void programsMenu_Remove(object sender, RoutedEventArgs e)
    {
        ...
        MenuBar._appGrabber.RemoveAppConfirm(app, (bool? result) => { MenuBar.OpenProgramsMenu(); });
    }
    ...
    private void ContextMenu_Opened(object sender, RoutedEventArgs e)
    {
        ...
        foreach (Control item in menu.Items)
        {
            ...
            {
                if ((string)mi.CommandParameter == "MoveUp" &&
                    category.ParentCategoryList.IndexOf(category) <= CategoryList.MIN_CATEGORIES)
                {
                    item.IsEnabled = false;
                }
                else if ((string)mi.CommandParameter == "MoveDown" && category.ParentCategoryList.IndexOf(category) >=
                         category.ParentCategoryList.Count - 1)
                {
                    item.IsEnabled = false;
                }
            }
    ...
    private void categoryMenu_Delete(object sender, RoutedEventArgs e)
    {
        ...
        CairoMessage.ShowOkCancel(
            string.Format(DisplayString.sProgramsMenu_DeleteCategoryInfo, category.DisplayName,
                MenuBar._appGrabber.CategoryList.GetSpecialCategory(AppCategoryType.All)),
