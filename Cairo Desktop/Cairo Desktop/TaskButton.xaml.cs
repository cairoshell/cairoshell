...
{
    public static readonly DependencyProperty ListModeProperty =
        DependencyProperty.Register("ListMode", typeof(bool), typeof(TaskButton), new PropertyMetadata(new bool()));

    public static DependencyProperty ParentTaskbarProperty =
        DependencyProperty.Register("ParentTaskbar", typeof(Taskbar), typeof(TaskButton));
    ...
    {
        if (ListMode || (IconSize)Settings.Instance.TaskbarIconSize == IconSize.Small ||
            !Settings.Instance.ShowTaskbarBadges)
        {
    ...
    private void ContextMenu_Opening(object sender, RoutedEventArgs e)
    {
        ...
        if (!_isGroup)
        {
            ...
            miMaximize.IsEnabled = wss != NativeMethods.WindowShowStyle.ShowMaximized &&
                                   (ws & (int)NativeMethods.WindowStyles.WS_MAXIMIZEBOX) != 0;
            miMinimize.IsEnabled = wss != NativeMethods.WindowShowStyle.ShowMinimized &&
                                   (ws & (int)NativeMethods.WindowStyles.WS_MINIMIZEBOX) != 0;
            ...
            miSize.IsEnabled = wss == NativeMethods.WindowShowStyle.ShowNormal &&
                               (ws & (int)NativeMethods.WindowStyles.WS_MAXIMIZEBOX) != 0;
        }
    }
    ...
    private void miPin_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (toPin != null)
        {
            ParentTaskbar._appGrabber.QuickLaunchManager.AddToQuickLaunch(toPin.IsUWP,
                toPin.IsUWP ? toPin.AppUserModelID : toPin.WinFileName);
        }
    }
