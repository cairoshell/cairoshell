...
public sealed class WindowManager : IWindowManager, IDisposable
{
    ...
    public static System.Drawing.Size PrimaryMonitorSize
    {
        get
        {
            return new System.Drawing.Size(
                Convert.ToInt32(SystemParameters.PrimaryScreenWidth / DpiHelper.DpiScaleAdjustment),
                Convert.ToInt32(SystemParameters.PrimaryScreenHeight / DpiHelper.DpiScaleAdjustment));
        }
    }
    ...
    public static System.Drawing.Size PrimaryMonitorWorkArea
    {
        get
        {
            return new System.Drawing.Size(SystemInformation.WorkingArea.Right - SystemInformation.WorkingArea.Left,
                SystemInformation.WorkingArea.Bottom - SystemInformation.WorkingArea.Top);
        }
    }
    ...
    private bool HaveDisplaysChanged()
    {
        ...
        if (ScreenState.Count == Screen.AllScreens.Length)
        {
            ...
            for (int i = 0; i < Screen.AllScreens.Length; i++)
            {
                ...
                if (!(ScreenState[i].Bounds == current.Bounds && ScreenState[i].DeviceName == current.DeviceName &&
                      ScreenState[i].Primary == current.Primary && ScreenState[i].WorkingArea == current.WorkingArea))
                {
                    same = false;
                    break;
                }
    ...
    private List<string> GetScreenDeviceNames()
    {
        ...
        {
            _logger.LogDebug(
                $"{screen.DeviceName} found at {screen.Bounds} with area {screen.WorkingArea}; primary? {screen.Primary}");
