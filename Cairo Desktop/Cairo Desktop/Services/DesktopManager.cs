...
public class DesktopManager : IDesktopManager, INotifyPropertyChanged, IDisposable
{
    ...
    public bool AllowProgmanChild =>
        ShellWindow == null &&
        EnvironmentHelper.IsWindows8OrBetter; // doesn't work in win7 due to layered child window restrictions

    public ShellFolder DesktopLocation => DesktopIconsControl?.Location;

    public DesktopManager(ILogger<DesktopManager> logger, ICairoApplication cairoApplication,
        ShellManagerService shellManagerService, ISettingsUIService settingsUiService)
    {
    ...
    private void CreateToolbar()
    {
        ...
        {
            DesktopToolbar = new DesktopNavigationToolbar(_cairoApplication, this)
                { Owner = DesktopWindow, NavigationManager = NavigationManager };
            DesktopToolbar.Show();
        }
        else if (Settings.Instance.EnableDynamicDesktop &&
                 DesktopWindow == null) // NavigationManager can be expected null
        {
    ...
    public void ConfigureDesktop()
    {
        if (DesktopWindow != null)
        {
            if (ShellWindow != null || AllowProgmanChild)
            {
                ...
                NativeMethods.SetWindowLong(DesktopWindow.Handle, NativeMethods.GWL_STYLE,
                    (NativeMethods.GetWindowLong(DesktopWindow.Handle, NativeMethods.GWL_STYLE) |
                     (int)NativeMethods.WindowStyles.WS_CHILD) &
                    ~unchecked((int)NativeMethods.WindowStyles.WS_OVERLAPPED));
                NativeMethods.SetParent(DesktopWindow.Handle,
                    ShellWindow?.Handle ?? WindowHelper.GetLowestDesktopChildHwnd());
            }
