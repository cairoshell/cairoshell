...
public partial class DesktopIcons : UserControl
{
    ...
    {
        get { return GetValue(locationProperty) as ShellFolder; }
        set
        {
    ...
    {
        RenderTargetBitmap bmp = new RenderTargetBitmap((int)(sourceGrid.ActualWidth * DpiHelper.DpiScale),
            (int)(sourceGrid.ActualHeight * DpiHelper.DpiScale), DpiHelper.DpiScale * 96, DpiHelper.DpiScale * 96,
    ...
    private bool InvokeContextMenu(ShellFile file, bool isInteractive)
    {
        ...
        var _ = new ShellItemContextMenu(new ShellItem[] { file }, _desktopManager.DesktopLocation, desktopPtr,
            HandleFileAction, isInteractive, GetFileCommandBuilder(file), new ShellMenuCommandBuilder());
        return true;
    }
    ...
    private bool HandleFileAction(string action, ShellItem[] items, bool allFolders)
    {
        ...
        switch (action)
        {
            ...
            default:
                ...
                else if (action != CustomCommands.Actions.Cut && action != CustomCommands.Actions.Copy &&
                         action != CustomCommands.Actions.Delete)
                {
                    // these actions are handled for us, but we should hide the desktop overlay
                    _desktopManager.IsOverlayOpen = false;
                }
