...
public partial class Desktop : Window, INotifyPropertyChanged
{
    ...
    public Desktop(IDesktopManager desktopManager, AppBarManager appBarManager, FullScreenHelper fullScreenHelper,
                ...
                if ((wndPos.flags & NativeMethods.SetWindowPosFlags.SWP_NOZORDER) == 0)
                {
                    ...
                    if (lowestHwnd != IntPtr.Zero)
                    {
                        wndPos.hwndInsertAfter =
                            NativeMethods.GetWindow(lowestHwnd, NativeMethods.GetWindow_Cmd.GW_HWNDPREV);
                    }
                    else
                    {
    ...
    {
        if (e.OriginalSource.GetType() == typeof(ScrollViewer) && (_desktopManager.DesktopToolbar == null ||
                                                                   !_desktopManager.DesktopToolbar.IsContextMenuOpen))
        {
    ...
    private void CairoDesktopWindow_MouseRightButtonUp(object sender, MouseButtonEventArgs e)
    {
        ...
        if (_desktopManager.DesktopLocation != null && (e.OriginalSource.GetType() == typeof(ScrollViewer) ||
                                                        e.Source.GetType() == typeof(Desktop) ||
                                                        e.Source.GetType() == typeof(Grid)))
        {
            ShellFolderContextMenu cm = new ShellFolderContextMenu(_desktopManager.DesktopLocation, HandleFolderAction,
                GetFolderCommandBuilder());
            e.Handled = true;
        }
    }
    ...
    private void FullScreenApps_CollectionChanged(object sender,
    ...
    private void setSize()
    {
        ...
        Height = (SystemInformation.VirtualScreen.Height / DpiHelper.DpiScale) -
                 (EnvironmentHelper.IsAppRunningAsShell
                     ? 0
                     : 1); // making size of screen causes explorer to send ABN_FULLSCREENAPP
    }
    ...
    private void setGridPosition()
    {
        ...
        if (Settings.Instance.TaskbarMode == 1)
        {
            ...
            grid.Height = (WindowManager.PrimaryMonitorWorkArea.Height / DpiHelper.DpiScale) -
                          ((screen.Bounds.Bottom - workAreaRect.Bottom) / dpiScale);
    ...
    private Brush GetCairoBackgroundBrush_Color()
    {
        ...
        {
            string regColor =
                Registry.GetValue(@"HKEY_CURRENT_USER\Control Panel\Colors", "Background", "0 0 0") as string;
    ...
    private Brush GetCairoBackgroundBrush_BingImageOfTheDay()
    {
        ...
        try
        {
            SupportingClasses.BingPhotoOfDayClient.BingWallPaperClient client =
                new SupportingClasses.BingPhotoOfDayClient.BingWallPaperClient();
            client.DownLoad();
            ...
            switch (wallpaperStyle)
            {
                case CairoWallpaperStyle.Tile:
                    ...
                    backgroundImageBrush.TileMode = TileMode.Tile;
                    backgroundImageBrush.Stretch =
                        Stretch.Fill; // stretch to fill viewport, which is pixel size of image, as WPF is DPI-aware
                    backgroundImageBrush.Viewport = new Rect(0, 0,
                        (backgroundImageBrush.ImageSource as BitmapSource).PixelWidth,
                        (backgroundImageBrush.ImageSource as BitmapSource).PixelHeight);
                    backgroundImageBrush.ViewportUnits = BrushMappingMode.Absolute;
    ...
    {
        if (_desktopManager.DesktopLocation != null && _desktopManager.DesktopLocation.IsFileSystem &&
            (e.Data.GetDataPresent(DataFormats.FileDrop) || e.Data.GetDataPresent(typeof(ShellItem))))
        {
