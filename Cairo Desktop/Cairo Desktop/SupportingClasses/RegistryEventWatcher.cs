...
public class RegistryEventWatcher : IDisposable
{
    ...
    {
        if (_handler == null)
            ShellLogger.Warning("RegistryEventWatcher: Registry event watcher started, but no handler has been set");
    ...
    private String GetQuery()
    {
        ...
        return
            $"SELECT * FROM RegistryValueChangeEvent WHERE Hive = '{REGISTRY_HIVE}' AND KeyPath = '{currentUser.User.Value}\\\\{REGISTRY_KEY}' AND ValueName = '{REGISTRY_VALUE}'";
    }
