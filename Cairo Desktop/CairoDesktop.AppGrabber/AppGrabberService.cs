...
public class AppGrabberService : DependencyObject, IAppGrabber, IDisposable
{
    ...
    private static DependencyProperty programsListProperty = DependencyProperty.Register("ProgramsList",
        typeof(List<ApplicationInfo>), typeof(AppGrabberService), new PropertyMetadata(new List<ApplicationInfo>()));

    public static AppGrabberUI uiInstance;

    private static readonly string[] excludedNames =
    {
        "documentation", "help", "install", "more info", "read me", "read first", "readme", "remove", "setup",
        "what's new", "support", "on the web", "safe mode"
    };
    ...
    {
        ConfigFile = configFile ?? Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) +
            @"\CairoAppConfig.xml";
    ...
    private void getPinnedApps()
    {
        ...
        string pinnedPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) +
    ...
    public static ApplicationInfo PathToApp(string filePath, string fileDisplayName, bool allowNonApps,
        bool allowExcludedNames)
    {
        ...
        try
        {
            ...
            if (fileExt.Equals(".lnk", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    ...
                    link.GetPath(builder, 260, out ManagedShell.ShellFolders.Structs.WIN32_FIND_DATA pfd,
                        ManagedShell.ShellFolders.Enums.SLGP_FLAGS.SLGP_RAWPATH);
            ...
            if (!allowNonApps)
            {
                if (!string.IsNullOrEmpty(target) &&
                    !ExecutableExtensions.Contains(Path.GetExtension(target), StringComparer.OrdinalIgnoreCase))
                {
    ...
    public void LaunchProgram(ApplicationInfo app)
    {
        ...
        else
        {
            ...
            if (app.IsStoreApp && app.AllowRunAsAdmin)
            {
                ...
                {
                    CairoMessage.Show(DisplayString.sError_FileNotFoundInfo, DisplayString.sError_OhNo,
                        MessageBoxButton.OK, CairoMessageImage.Error);
                }
            }
    ...
    public void ShowAppProperties(ApplicationInfo app)
    {
        ...
        if (app.IsStoreApp)
            CairoMessage.Show(DisplayString.sProgramsMenu_UWPInfo, app.Name, app.GetIconImageSource(IconSize.Jumbo),
                true);
        else
        ...
        foreach (string fileName in fileNames)
        {
            ...
            if (categoryType == AppCategoryType.Uncategorized || categoryType == AppCategoryType.Standard)
            {
                ...
                if (CategoryList.FlatList.Contains(customApp))
                {
                    ...
                    ShellLogger.Debug(
                        $"AppGrabberService: Excluded duplicate item: {customApp.Name}: {customApp.Target}");
                    continue;
                }
            }
            else
