...
{
    public static IServiceCollection LoadDependencies(this IServiceCollection serviceCollection, string path,
        string pattern = null)
    {
        ...
        try
        {
            using (var aggregateCatalog = new AggregateCatalog())
            {
                ...
                using (var compositionContainer = new CompositionContainer(aggregateCatalog))
                {
                    ...
                    IEnumerable<IDependencyRegistrant> modules =
                        exports.Select(export => export.Value as IDependencyRegistrant).Where(m => m != null);
    ...
    private static ImportDefinition BuildImportDefinition()
    {
        return new ImportDefinition(
            exportDefinition => true, typeof(IDependencyRegistrant).FullName, ImportCardinality.ZeroOrMore,
            isRecomposable: false, isPrerequisite: false);
    }
}