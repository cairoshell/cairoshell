...
{
    private readonly string stackConfigFile = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) +
                                              @"\CairoStacksConfig.xml";

    private System.Xml.Serialization.XmlSerializer serializer;

    private InvokingObservableCollection<ShellFolder> _stackLocations =
        new InvokingObservableCollection<ShellFolder>(Application.Current.Dispatcher);
    ...
    private void deserializeStacks()
    {
        ...
        else
        {
            ...
            AddLocation(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments,
